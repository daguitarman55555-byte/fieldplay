...
describe('natural calculus notation templates',()=>{
  ...
  it('accepts the fraction Desmos creates from typed partial shorthand',()=>{expect(notationTemplate('\\frac{pdf}{pdx}').latex).toBe('\\frac{∂f}{∂x}');expect(notationTemplate('\\frac{dpf}{dpx}').latex).toBe('\\frac{∂f}{∂x}');});
  it('accepts shorthand pasted as plain slash notation',()=>{expect(notationTemplate('pdf/pdx').latex).toBe('\\frac{∂f}{∂x}');expect(notationTemplate('dpf/dpx').latex).toBe('\\frac{∂f}{∂x}');expect(notationTemplate('d/dx').latex).toBe('\\frac{d}{dx}\\left(\\right)');});
  ...
  it('formats vector-calculus operators',()=>{expect(notationTemplate('grad').latex).toBe('∇\\left(\\right)');expect(notationTemplate('div').latex).toContain('∇\\cdot');});

export function notationTemplate(latex){
  ...
  let match=/(.*)\\frac\{(?:pd|dp)([^{}]+)\}\{(?:pd|dp)([A-Za-z])\}$/.exec(latex);if(match)return{latex:`${match[1]}\\frac{\\partial ${match[2]}}{\\partial ${match[3]}}`,left:0};
  ...
  const operators=[['jacobian','\\operatorname{jacobian}\\left(\\left[\\right],\\left[x,y\\right]\\right)'],['hessian','\\operatorname{hessian}\\left(,\\left[x,y\\right]\\right)'],['laplacian','\\nabla^{2}\\left(\\right)'],['magnitude','\\left\\|\\right\\|'],['grad','\\nabla\\left(\\right)'],['curl','\\nabla\\times\\left(\\right)'],['div','\\nabla\\cdot\\left(\\right)']];for(const[command,template]of operators)if(endsCommand(latex,command))return{latex:`${latex.slice(0,-command.length)}${template}`,left:command==='jacobian'?6:command==='hessian'?5:1};

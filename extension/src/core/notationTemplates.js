export function notationTemplate(latex){
  ...
  const operators=[['laplacian','\\nabla^{2}\\left(\\right)'],['magnitude','\\left\\|\\right\\|'],['grad','\\nabla\\left(\\right)'],['curl','\\nabla\\times\\left(\\right)'],['div','\\nabla\\cdot\\left(\\right)']];for(const[command,template]of operators)if(endsCommand(latex,command))return{latex:`${latex.slice(0,-command.length)}${template}`,left:1};
